...
namespace Devolutions.Authenticode.PowerShell
{
                ...
                foreach (string p in FilePath)
                {
                    ...
                    foreach (string path in paths)
                    {
                        if (!System.IO.Directory.Exists(path))
                        {
                            ...
                            string resolvedFilePath = path; // FIXME
    ...
    public sealed class SetAuthenticodeSignatureCommand : SignatureCommandsBase
    {
        ...
        protected override Signature PerformAction(string filePath)
        {
            ...
            //
            if (!Authenticode.SecuritySupport.CertIsGoodForSigning(Certificate))
            {
                /*
            ...
            try
            {
                ...
                if (Authenticode.Utils.GetFileSize(filePath) < 4)
                {
                    /*
        ...
        {
            internal Authenticode.SigningOption option;
            internal string optionName;

            internal SigningOptionInfo(Authenticode.SigningOption o, string n)
            {
                option = o;
                optionName = n;
            }
        ...
        {
            new SigningOptionInfo(Authenticode.SigningOption.AddOnlyCertificate, "signer"),
            new SigningOptionInfo(Authenticode.SigningOption.AddFullCertificateChainExceptRoot, "notroot"),
            new SigningOptionInfo(Authenticode.SigningOption.AddFullCertificateChain, "all")
        };
        ...
        private static Authenticode.SigningOption GetSigningOption(string optionName)
        {
            ...
            return Authenticode.SigningOption.AddFullCertificateChainExceptRoot;
        }
    }
}
